...
class SQuadDataset(tf.data.Dataset):
    ...
    def context2ids(self, sequence, word2idx):
        ...
        ids = tf.Tensor(ids, dtype=tf.int64)
        extended_ids = tf.Tensor(extended_ids, dtype=tf.int64)
...
def collate_fn(data):
    def merge(sequences):
        ...
        padded_seqs = tf.zeros([len(sequences), max(lengths)])#.long()
